...
const API_URL = "/api"; // kein localhost nötig, weil alles vom gleichen Server kommt
...
async function addUser() {
    ...
    try {
        const response = await fetch(`${API_URL}/users`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ username, password, dienummer, rank, role })
        });
        ...
        console.error(err);
        alert("❌ Netzwerkfehler – konnte Benutzer nicht hinzufügen.");
    }
}
...
async function removeUser(userId) {
    ...
    if (user.id === currentUser.id) {
        alert('⚠️ Sie können sich nicht selbst entfernen.');
        return;
    }
        ...
        await sendWebhook(WEBHOOKS.userRemoved, {
            embeds: [{
                ...
                fields: [
                    ...
                    { name: 'Diennummer', value: user.dienummer, inline: true },
